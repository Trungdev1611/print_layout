    ...
    public class PrintOptionsDialog : Form
    {
        ...
        private void LoadValues()
        {
            ...
            _plotAreaCombo.SelectedIndex = string.Equals(lastPlotArea, "Extents", StringComparison.OrdinalIgnoreCase)
                ? 1
                : 0;
            // Last chosen file/folder if still on disk; otherwise next to the DWG.
        ...
        private static string ResolveOutputPath(string remembered, string freshDefault)
        {
            ...
            try
            {
                if (File.Exists(remembered) || Directory.Exists(remembered))
                    return remembered;
                var dir = Path.GetDirectoryName(remembered);
                if (!string.IsNullOrWhiteSpace(dir) && Directory.Exists(dir))
                    return remembered;
            }
            catch { }
            return freshDefault ?? remembered ?? "";
        }
